...
class LLMClient:
    def __init__(self, settings: LLMSettings | None = None):
        ...
        if self.settings.provider in ("openai", "ollama", "gemini", "huggingface", "hf"):
            kwargs: dict[str, Any] = {"api_key": self.settings.api_key or "demo-key"}
        ...
        if self.settings.provider in ("huggingface", "hf") and not self.settings.api_key:
            raise ValueError("LLM_PROVIDER=huggingface cần HUGGINGFACE_API_KEY hoặc HF_TOKEN")
    ...
    def chat_completions_stream(
        ...
        **_kwargs: Any,
        ...
        """
        Yields text chunks as they are streamed from LLM.
        Supports Qwen (GPU local), Gemini, OpenAI, etc.
        """
        if self.settings.provider == "qwen":
            from .qwen_service import get_qwen_service
            qwen = get_qwen_service()
            for chunk in qwen.generate_stream(messages, max_new_tokens=max_tokens, temperature=temperature):
                yield chunk
            return
    ...
    def _qwen_chat(
        ...
        # 1. Nếu có URL endpoint Qwen từ xa, gọi qua HTTP
        if self.settings.qwen_api_url:
        ...
        # 2. Ngược lại, chạy trực tiếp trên GPU thông qua QwenService
        from .qwen_service import get_qwen_service
        qwen = get_qwen_service()
        reply = qwen.generate(messages, max_new_tokens=max_tokens, temperature=temperature)
        return _wrap_chat_content(reply)
